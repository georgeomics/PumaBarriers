...
predC <- "ecoregs_" # useful for file naming
...
# ======================== PATCH DATA ================================
# Load and clean up data so that variable is defined in the dataset and 'Q' is a matrix or dataframe of observed probabilities
...
stan_data <- list(
  ...
  PC = ncol(dummy_mat_predC), # number of predictors (e.g., ecoregion dummies)
  roads = dummy_mat_predA,
  rivers = dummy_mat_predB,
  ecoregs = dummy_mat_predC,
...
fit <- stan(file = paste0(stan_dir, "full.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
# Nonspatial
# fit <- stan(file = paste0(stan_dir, "/nonspatial/full_NS.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)

# save fit
saveRDS(fit, file = paste0(output_dir, K_prefix, predA, predB, predC, "fit.rds"))
