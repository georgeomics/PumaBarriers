...
# OUTPUT SUMMARY
library(rethinking)
sink(paste0(output_dir, K_prefix, predA, "precis.txt")) # like 'pdf()' but for text files
...
sink(paste0(output_dir, K_prefix, predA, "loo.txt"))  # like 'pdf()' but for text files
print(loo_result)
sink()


# ======================== PREDICTOR COLLINEARITY ================================
cor_mat <- cor(dummy_mat) # make correlation matrix,
print(cor_mat)
# Variance Inflation Factor (VIF)
library(car)
dummy_df <- as.data.frame(dummy_mat) # convert matrix to df
vif_model <- lm(Q[,1] ~ ., data = dummy_df) # predA with Q[,1] as response and all columns in dummy_df as preds (necessary for VIF)
vif_vals <- vif(vif_model) # get VIFs

sink(paste0(output_dir, K_prefix, predA, "VIF_results.txt"))
print(vif_vals)
sink()
...
# quantiles summary
sink(paste0(output_dir, K_prefix, predA, "std_resids_quantiles_pop", pop, ".txt"))
quantile(std_resids_pop, probs = c(0, 0.25, 0.5, 0.75, 1)) # between -2 and 2 is generally ideal
sink()
# Q-Q plot
pdf(paste0(output_dir, K_prefix, predA, "qqnorm_pop", pop, ".pdf"))
...
sink(paste0(output_dir, K_prefix, predA, "MoransI_pop", pop, ".txt"))
print(moran_result)
sink()
##### ##### #####
...
pdf(paste0(output_dir, K_prefix, predA, "ppc_mean_Pop1.pdf"))
ppc_stat(y=Q_obs_pop1, yrep=Q_pred_pop1_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "ppc_mean_Pop2.pdf"))
ppc_stat(y=Q_obs_pop2, yrep=Q_pred_pop2_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "ppc_mean_Pop3.pdf"))
ppc_stat(y=Q_obs_pop3, yrep=Q_pred_pop3_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "ppc_mean_Pop4.pdf"))
...
# Print 
sink(paste0(output_dir, K_prefix, predA, "bayesian_p_values.txt"))  # like 'pdf()' but for text files
...
########## MCMC trace plots ##########
pdf(paste0(output_dir, K_prefix, predA, "trace_plot_beta0.pdf"))
stan_trace(fit, pars = c("beta0"), nrow = 6, ncol = 4) +
  theme(axis.text.x = element_text(size = 7), axis.text.y = element_text(size = 7)) 
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "trace_plot_beta.pdf"))
stan_trace(fit, pars = c("beta"), nrow = 6, ncol = 4) +
  theme(axis.text.x = element_text(size = 7), axis.text.y = element_text(size = 7)) 
dev.off()


pdf(paste0(output_dir, K_prefix, predA, "trace_plot_phi.pdf"))
stan_trace(fit, pars = c("phi"))
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "trace_plot_phi_Dirich.pdf"))
stan_trace(fit, pars = c("phi_Dirich"))
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "trace_plot_sigmasq.pdf"))
...
########## Fit plot ##########
pdf(paste0(output_dir, K_prefix, predA, "fit_plot_beta0.pdf"))
plot(fit, pars=c("beta0"))
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "fit_plot_beta.pdf"))
plot(fit, pars=c("beta"))
dev.off()


pdf(paste0(output_dir, K_prefix, predA, "fit_plot_phi.pdf"))
plot(fit, pars=c("phi"))
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "fit_plot_phi_Dirich.pdf"))
plot(fit, pars=c("phi_Dirich"))
dev.off()

pdf(paste0(output_dir, K_prefix, predA, "fit_plot_sigmasq.pdf"))
...
#   pdf(paste0(output_dir, K_prefix, predA, "pairs_plot_beta0_", i, ".pdf"), width = 10, height = 10)
#   pairs(fit, pars = paste0("beta0[", i, "]"), las = 1)
#   dev.off()
# }

for (i in 1:3) { # for each row of beta
  param_names <- paste0("beta[", i, ",", 1:4, "]")
  pdf(paste0(output_dir, K_prefix, predA, "pairs_plot_beta_", i, ".pdf"), width = 10, height = 10)
  pairs(fit, pars = param_names, las = 1)
  dev.off()
}

# pdf(paste0(output_dir, K_prefix, predA, "pairs_plot_lambda.pdf"), width = 10, height = 10)
# pairs(fit, pars = "lambda", las = 1) 
# dev.off()

# pdf(paste0(output_dir, K_prefix, predA, "pairs_plot_phi.pdf"), width = 10, height = 10)
# pairs(fit, pars = "phi", las = 1) 
# dev.off()
