...
dummy_mat <- model.matrix(~ rivers, data = patch_data)[, -1, drop=FALSE]
...
fit <- stan(file = paste0(stan_dir, "riv.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
# Nonspatial:
# fit <- stan(file = paste0(stan_dir, "/nonspatial/riv_NS.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
