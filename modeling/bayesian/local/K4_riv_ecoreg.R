...
# model
model <- "riv_ecoreg_" # for barplots
...
fit <- stan(file = paste0(stan_dir, "riv_ecoreg_RE.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
# print(fit)

saveRDS(fit, file = paste0(output_dir, K_prefix, "riv_ecoreg_RE.rds"))
...
sink(paste0(output_dir, K_prefix, "riv_ecoreg_precis.txt")) # like 'pdf()' but for text files
precis(fit, depth=3)
sink()
...
sink(paste0(output_dir, K_prefix, "riv_ecoreg_loo.txt"))  # like 'pdf()' but for text files
print(loo_result)
sink()
...
pdf(paste0(output_dir, K_prefix, "riv_ecoreg_ppc_mean_Pop1.pdf"))
ppc_stat(y=Q_obs_pop1, yrep=Q_pred_pop1_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_ppc_mean_Pop2.pdf"))
ppc_stat(y=Q_obs_pop2, yrep=Q_pred_pop2_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_ppc_mean_Pop3.pdf"))
ppc_stat(y=Q_obs_pop3, yrep=Q_pred_pop3_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_ppc_mean_Pop4.pdf"))
...
# Print 
sink(paste0(output_dir, K_prefix, "riv_ecoreg_bayesian_p_values.txt"))  # like 'pdf()' but for text files
...
########## MCMC trace plots ##########
pdf(paste0(output_dir, K_prefix, "riv_ecoreg_trace_plot_beta_river.pdf"))
stan_trace(fit, pars = c("beta_river"))
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_trace_plot_beta_ecoreg.pdf"))
stan_trace(fit, pars = c("beta_ecoreg"))
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_trace_plot_gamma.pdf"))
stan_trace(fit, pars = c("gamma"))
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_trace_plot_epsilon.pdf"))
stan_trace(fit, pars = c("epsilon"))
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_trace_plot_mu.pdf"))
...
########## Fit plot ##########
pdf(paste0(output_dir, K_prefix, "riv_ecoreg_fit_plot_beta_river.pdf"))
plot(fit, pars=c("beta_river"))
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_fit_plot_beta_ecoreg.pdf"))
plot(fit, pars=c("beta_ecoreg"))
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_fit_plot_gamma.pdf"))
plot(fit, pars=c("gamma"))
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_fit_plot_epsilon.pdf"))
plot(fit, pars=c("epsilon"))
dev.off()

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_fit_plot_mu.pdf"))
plot(fit, pars=c("mu"))
dev.off()
...
########## Pairs plots ##########
for (i in 1:K) {
  pdf(paste0(output_dir, K_prefix, "riv_ecoreg_pairs_plot_beta_river", i, ".pdf"), width = 10, height = 10)
  pairs(fit, pars = paste0("beta_river[", i, ",", 1:R, "]"), las = 1)
  dev.off()
}

for (i in 1:K) {
  pdf(paste0(output_dir, K_prefix, "riv_ecoreg_pairs_plot_beta_ecoreg", i, ".pdf"), width = 10, height = 10)
  pairs(fit, pars = paste0("beta_ecoreg[", i, ",", 1:E, "]"), las = 1)
  dev.off()
}

pdf(paste0(output_dir, K_prefix, "riv_ecoreg_pairs_plot_mu.pdf"), width = 10, height = 10)
