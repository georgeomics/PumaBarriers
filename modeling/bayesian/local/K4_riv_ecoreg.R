...
predA <- "rivers_" # useful for file naming
predB <- "ecoregs_" # useful for file naming
...
dummy_mat_predA <- model.matrix(~ rivers, data = patch_data)[, -1, drop=FALSE]
...
# Remove intercept (first group is base)
dummy_mat_predB <- model.matrix(~ US_L3NAME_num, data = patch_data)[, -1, drop=FALSE]
...
stan_data <- list(
  ...
  rivers = dummy_mat_predA,
  ecoregs = dummy_mat_predB,
...
fit <- stan(file = paste0(stan_dir, "riv_ecoreg.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
# Nonspatial:
# fit <- stan(file = paste0(stan_dir, "/nonspatial/riv_ecoreg_NS.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
