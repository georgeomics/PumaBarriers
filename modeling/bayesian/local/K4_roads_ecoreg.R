...
fit <- stan(file = paste0(stan_dir, "roads_ecoreg.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
# Nonspatial:
# fit <- stan(file = paste0(stan_dir, "/nonspatial/roads_ecoreg_NS.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
