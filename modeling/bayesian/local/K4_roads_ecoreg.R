...
# model
model <- "roads_ecoreg_" # for barplots
...
K <- 4    # number of ancestral groups
# R <- 5    # number of river "patches"
I <- 6    # number of road "patches"
E <- 5    # number of ecoreg "patches"
...
# ------------------------------------------------------------------------------------------------------------
# rivers <- patch_data$rivers
roads <- patch_data$roads
ecoreg <- patch_data$US_L3NAME_num # ecoreg
...
stan_data <- list(
  ...
  ecoreg = ecoreg,
  Z = Z,
#   adjacency_river = adjacency_river,
# #   adjacency_roads = adjacency_roads,
#   adjacency_ecoreg = adjacency_ecoreg,
  Q = as.matrix(Q)            # Make sure 'Q' is formatted correctly
)
...
fit <- stan(file = paste0(stan_dir, "roads_ecoreg_RE.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
# print(fit)

saveRDS(fit, file = paste0(output_dir, K_prefix, "roads_ecoreg_RE.rds"))
...
sink(paste0(output_dir, K_prefix, "roads_ecoreg_precis.txt")) # like 'pdf()' but for text files
precis(fit, depth=3)
sink()
...
sink(paste0(output_dir, K_prefix, "roads_ecoreg_loo.txt"))  # like 'pdf()' but for text files
print(loo_result)
sink()
...
# Add observation index and river category
# Q_obs <- Q_obs %>% mutate(obs = row_number(), river = factor(rivers))
...
# Q_obs_pop1 <- as.numeric(Q_obs$Pop1) # not working
...
pdf(paste0(output_dir, K_prefix, "roads_ecoreg_ppc_mean_Pop1.pdf"))
ppc_stat(y=Q_obs_pop1, yrep=Q_pred_pop1_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_ppc_mean_Pop2.pdf"))
ppc_stat(y=Q_obs_pop2, yrep=Q_pred_pop2_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_ppc_mean_Pop3.pdf"))
ppc_stat(y=Q_obs_pop3, yrep=Q_pred_pop3_corrected, stat = "mean")
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_ppc_mean_Pop4.pdf"))
...
# Print 
sink(paste0(output_dir, K_prefix, "roads_ecoreg_bayesian_p_values.txt"))  # like 'pdf()' but for text files
...
########## MCMC trace plots ##########
pdf(paste0(output_dir, K_prefix, "roads_ecoreg_trace_plot_beta_ecoreg.pdf"))
stan_trace(fit, pars = c("beta_ecoreg"))
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_trace_plot_beta_roads.pdf"))
stan_trace(fit, pars = c("beta_roads"))
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_trace_plot_gamma.pdf"))
stan_trace(fit, pars = c("gamma"))
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_trace_plot_epsilon.pdf"))
stan_trace(fit, pars = c("epsilon"))
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_trace_plot_mu.pdf"))
stan_trace(fit, pars = c("mu"))
dev.off()

graphics.off()

########## Fit plot ##########
pdf(paste0(output_dir, K_prefix, "roads_ecoreg_fit_plot_beta_ecoreg.pdf"))
plot(fit, pars=c("beta_ecoreg"))
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_fit_plot_beta_roads.pdf"))
plot(fit, pars=c("beta_roads"))
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_fit_plot_gamma.pdf"))
plot(fit, pars=c("gamma"))
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_fit_plot_epsilon.pdf"))
plot(fit, pars=c("epsilon"))
dev.off()

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_fit_plot_mu.pdf"))
plot(fit, pars=c("mu"))
dev.off()

########## Pairs plots ##########
for (i in 1:K) {
  pdf(paste0(output_dir, K_prefix, "roads_ecoreg_pairs_plot_beta_ecoreg", i, ".pdf"), width = 10, height = 10)
  pairs(fit, pars = paste0("beta_ecoreg[", i, ",", 1:E, "]"), las = 1)
  dev.off()
}

for (i in 1:K) {
  pdf(paste0(output_dir, K_prefix, "roads_ecoreg_pairs_plot_beta_roads", i, ".pdf"), width = 10, height = 10)
  pairs(fit, pars = paste0("beta_roads[", i, ",", 1:I, "]"), las = 1)
  dev.off()
}

pdf(paste0(output_dir, K_prefix, "roads_ecoreg_pairs_plot_mu.pdf"), width = 10, height = 10)
pairs(fit, pars = "mu", las = 1) 
dev.off()
