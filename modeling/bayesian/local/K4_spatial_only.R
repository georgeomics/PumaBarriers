...
# model
predA <- "spatial_only_" # for naming
...
fit <- stan(file = paste0(stan_dir, "spatial_only.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)

# save fit
saveRDS(fit, file = paste0(output_dir, K_prefix, "spatial_only_", "fit.rds"))
