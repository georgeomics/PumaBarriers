...
# -----------------------------------------------------------------------------------------------------------
# manually assign outlier pumas outisde of state boundary to road region 1
patch_data <- patch_data %>%
  mutate(roads = ifelse(Puma_ID == "UA00031261", 1, roads),
         roads = ifelse(Puma_ID == "UA00047644", 1, roads))
# ------------------------------------------------------------------------------------------------------------
...
dummy_mat <- model.matrix(~ roads, data = patch_data)[, -1, drop=FALSE]
...
fit <- stan(file = paste0(stan_dir, "roads.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
# Nonspatial:
# fit <- stan(file = paste0(stan_dir, "/nonspatial/roads_NS.stan"), data = stan_data, cores=detectCores(), chains = chains, iter = iter, warmup = warmup)
