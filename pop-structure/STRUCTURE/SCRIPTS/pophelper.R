...
evannoMethodStructure(data=sr1) # gives you a nice summary table of logprob, likelihood, etc

# # comma delineated printing (for easy table conversion)
ev <- evannoMethodStructure(data = sr1)
# grab whatever columns/stats you want from sr1
ev$elpdmean <- round(ev$elpdmean, 2)
ev$elpdsd   <- round(ev$elpdsd,   2)
ev$lnk1     <- round(ev$lnk1,     2)
ev$lnk2     <- round(ev$lnk2,     2)
ev$deltaK   <- round(ev$deltaK,   2)
write.table(ev[, c("k","elpdmean","elpdsd","lnk1","lnk2","deltaK")],
